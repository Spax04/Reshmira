...
import { Shift, ShiftWithId, shiftModel } from '../models/shift';
...
const getShift = (id: mongoose.Types.ObjectId) => {
    return new Promise<ShiftWithId>((resolve, reject) => {
        mongoose.connect(process.env.MONGO_URL as string).then(async () => {
            ...
            const foundShift = await shiftModel.findById(id).exec().catch((error) => {
                reject(error);
            });
...
const updateShift = (id: mongoose.Types.ObjectId, updatedShiftData: Partial<Shift>) => {
    return new Promise<ShiftWithId>((resolve, reject) => {
        mongoose.connect(process.env.MONGO_URL as string).then(async () => {
            try {
                const updatedShift = await shiftModel.findByIdAndUpdate(id, updatedShiftData, { new: true });
                if (!updatedShift) {
...
const deleteShift = (id: mongoose.Types.ObjectId) => {
    return new Promise<void>(async (resolve, reject) => {
        mongoose.connect(process.env.MONGO_URL as string).then(async () => {
            await shiftModel.findByIdAndDelete(id).exec().catch(err => {
                if (err) {
                    reject(err);
                }
            });
